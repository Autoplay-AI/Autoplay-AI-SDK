...
- Added dedicated references for [`UserSessionIndex`](/sdk/user-session-index), [`compose_chat_pipeline(...)`](/sdk/compose-chat-pipeline), and [`build_copilot_app(...)`](/sdk/build-copilot-app), including lifecycle details, endpoint status semantics, and failure troubleshooting.
