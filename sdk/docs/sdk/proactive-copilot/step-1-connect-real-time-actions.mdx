...
### When to use `UserSessionIndex`

As soon as your chat layer is keyed by `user_id` (not only `session_id`), add
[`UserSessionIndex`](../user-session-index) so one user can be mapped to recent
session refs and chat retrieval stays aligned with real activity.
